...
const wxLanguageInfo* SjTools::FindLanguageInfo(const wxString& canonicalName__)
{
	wxString canonicalName(canonicalName__);
	int i;
	const wxLanguageInfo* info;

	// find exact language/country pair
	for( i = 0; i < wxLANGUAGE_USER_DEFINED; i++ )
	{
		info = wxLocale::GetLanguageInfo(i);
		if( info && canonicalName.CmpNoCase(info->CanonicalName)==0 )
		{
			return info;
		}
	}
	...
	// find only language
	canonicalName = canonicalName.BeforeFirst('_');
	for( i = 0; i < wxLANGUAGE_USER_DEFINED; i++ )
	{
		info = wxLocale::GetLanguageInfo(i);
		if( info && canonicalName.CmpNoCase(info->CanonicalName.BeforeFirst('_'))==0 )
		{
			return info;
		}
	}
...
wxArrayString SjTools::GetAvailLanguages(wxArrayString* files)
{
	wxArrayString ret;

	// add the default language (en.mo) does not really exists;
	// instead these are the strings directly used in the source code
	ret.Add(wxT("en_GB"));
	if( files )
	{
		files->Add(wxT("memory:en.mo"));
	}

	// add languages from the search paths
	int searchPathIndex;
	for( searchPathIndex=0; searchPathIndex<GetSearchPathCount(); searchPathIndex++ )
	{
		wxString searchPath = GetSearchPath(searchPathIndex);
		wxString entry;
		entry = ::wxFindFirstFile(searchPath + wxT("/*.mo"), wxFILE);
		while( !entry.IsEmpty() )
		{
			wxString entryName = GetFileNameFromUrl(entry);
			entryName = entryName.BeforeFirst('.');
