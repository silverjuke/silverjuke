...
sj_token_t Parser::parseToken(std::istream &  fs, char * string)
{
	...
	if (!wrapsToNextLine(buffer.str())) {
		...
		for (int k = 0; k <= buf_size; k++) {
			if (fs)
				...
				return tEOL;//abort(); - EDIT BY SJ - everything, but not program abort!
		}
		return tEOL;
	}
...
GenExpr * Parser::parse_infix_op(std::istream &  fs, sj_token_t token, TreeExpr * tree_expr, MilkdropPreset * preset)
{
					...
					if (!wrapsToNextLine(buffer.str())) {
						...
						for (int k = 0; k <= buf_size; k++) {
							if (fs)
								...
								return; // abort(); -- EDIT BY SJ -- everything but no program abort!
						}
						return;
					}
