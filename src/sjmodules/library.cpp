...
bool SjLibraryModule::FirstLoad()
{
...

	{
		...
		// create source table, if not exits
		// (this table was introduces in Silverjuke 1.17beta4)
#undef ADD_SOURCE_TABLE
#ifdef ADD_SOURCE_TABLE
		if( !sql.TableExists(wxT("sources")) )
		{
			sql.Query(wxT("CREATE TABLE sources ("
			...
			sql.AddColumn("tracks", "sourceid INTEGER DEFAULT 0");

			// For backward compatibility, add sources from the folderscanner-INI as used before 1.17beta4.
			// This stuff was added on 07.12.2005.
			wxBusyCursor busy;
			SjBusyInfo::Set(_("Updating music library")+wxString("..."), TRUE);

			wxConfigBase* c = g_tools->m_config;
			int i, sourceCount = c->Read("folderscanner/sourceCount", 0L);
			for( i = 0; i < sourceCount; i++ )
			{
				wxString currUrl = c->Read(wxString::Format("folderscanner/url%i", i), "");
				if( !currUrl.IsEmpty() )
				{
#define SJ_FOLDERSCANNER_DEFFLAGS 0x0000FFFFL
					long currFlags = c->Read(wxString::Format("folderscanner/flags%i", i), SJ_FOLDERSCANNER_DEFFLAGS);

					SjSSHash options;
					options.Insert("ignoreExt", c->Read(wxString::Format("folderscanner/ignoreExt%i", i), ""));
					options.Insert("pattern", c->Read(wxString::Format("folderscanner/pattern%i", i), SjTrackInfoMatcher::GetDefaultPattern()));

					wxString currFile = c->Read(wxString::Format("folderscanner/file%i", i), "");
					if( !currFile.IsEmpty() )
					{
						wxFileName fn(currUrl, currFile);
						fn.Normalize();
						currUrl = fn.GetFullPath();
						options.Insert("isFile", "1");
					}

					sql.Query("INSERT INTO sources (scanner, url, flags, options) "
					          "VALUES ('folderscanner.lib', '" + sql.QParam(currUrl) + "', " + sql.UParam(currFlags) + ", '" + sql.QParam(options.Serialize()) + "');");

					long newSourceId = sql.GetInsertId();
					wxString tracksUrlBeg = currUrl;
					tracksUrlBeg.Replace("\\", "/");
					sql.Query("UPDATE tracks SET sourceid=" + sql.UParam(newSourceId) + " WHERE url LIKE '" + sql.QParam(tracksUrlBeg) + "%';");
				}
			}

			// same for the servers, may be removed soon as this was only spreaded to broadchart.com
			sourceCount = sql.ConfigRead("serverscanner/sCount", 0L);
			for( i = 0; i < sourceCount; i++ )
			{
				wxString currServerName = sql.ConfigRead(wxString::Format("serverscanner/s%iname", i), "");
				if( !currServerName.IsEmpty() )
				{
					long currFlags = sql.ConfigRead(wxString::Format("serverscanner/s%iflags", i), SJ_FOLDERSCANNER_DEFFLAGS);;

					SjSSHash options;
					options.Insert("type", "cvsOverHttp");
					options.Insert("loginName", sql.ConfigRead(wxString::Format("serverscanner/s%iloginName", i), ""));
					options.Insert("loginPw", sql.ConfigRead(wxString::Format("serverscanner/s%iloginPw", i), ""));

					sql.Query("INSERT INTO sources (scanner, url, flags, options) "
					          "VALUES ('serverscanner.lib', '" + sql.QParam(currServerName) + "', " + sql.UParam(currFlags) + ", '" + sql.QParam(options.Serialize()) + "');");

					long newSourceId = sql.GetInsertId();
					wxString tracksUrlBeg = "http://"+currServerName;
					sql.Query("UPDATE tracks SET sourceid=" + sql.UParam(newSourceId) + " WHERE url LIKE '" + sql.QParam(tracksUrlBeg) + "%';");
				}
			}
		}
